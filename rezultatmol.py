...
# %% 
# 9. Vizualizare substructuri moleculare pentru top SHAP features (doar activate) - subset 3 molecule
subset_size = 3

# Alegem 3 molecule aleatorii din X_test
subset_idx = np.random.choice(len(X_test), size=subset_size, replace=False)
X_subset = X_test[subset_idx]

# Extragem SMILES corespunzătoare
smiles_subset = data.iloc[valid_idx].iloc[subset_idx]['smiles'].values

# Calculăm SHAP values pentru Random Forest
rf_model = models['RandomForest']
rf_explainer = shap.TreeExplainer(rf_model)
rf_shap_values = rf_explainer.shap_values(X_subset, check_additivity=False)
shap_matrix = get_shap_matrix(rf_shap_values)

# Selectăm top 5 feature-uri SHAP per moleculă
...
for mol_idx, mol_array in enumerate(top_features_idx):
    ...
    for bit in active_bits:
        ...
        for atom_idx, radius in bitInfo[bit]:
            ...
            plt.figure()
            plt.title(f"Bit {bit} (atom {atom_idx}, radius {radius})")
            plt.imshow(img)
            plt.axis('off')
            plt.show()
